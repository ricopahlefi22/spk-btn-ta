                    ...
                    <tbody class="table-border-bottom-0">
                      ...
                      <tr>
                        ...
                        <td>
                          <div class="row">
                            <div class="btn-group">
                              <form action="{{url('Karyawan/perhitungan/hapus', $perhitungan->id)}}" method="post" class="form-inline" style="margin-right: 5px" onsubmit="return confirm('Yakin Akan Menghapus Data Ini?')">
                                @csrf
                                @method("delete")
                                <button class="btn btn-danger"><i class="bx bx-user-x"></i></button>
                              </form>
                              <a href="{{url('Karyawan/tambah-bobot', $perhitungan->id)}}"><button class="btn btn-warning"><i class="bx bx-archive-in"></i> Bobot</button></a>
                            </div>
                          </div>
                        </td>
                        @else
                        <td>
                          <form action="{{url('Karyawan/perhitungan/hapus', $perhitungan->id)}}" method="post" class="form-inline" onsubmit="return confirm('Yakin Akan Menghapus Data Ini?')">
                            @csrf
                            @method("delete")
                            <button class="btn btn-danger"><i class="bx bx-user-x"></i></button>
                          </form>
